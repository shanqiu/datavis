...
void testApp::setup() {
    ...
    intro.loadSound("sounds/intro3.wav");
...
void testApp::draw(){
    ...
    for (int i = 0; i < numUsers; i++){
       ...
       //left_foot = user.getJoint(JOINT_LEFT_FOOT).getProjectivePosition();
        ...
        if(currTime-firstDelay>1000){
        if(left_hand.y<60 && level==0){
            introOk=false;
        ...
        if(diff_left_hand >20&& startCount==true){
        ...
        if(diff_left_hip_x>30 || diff_left_hip_z >30){
            if(level==1 || level==2){
                ofSetColor(255, 0, 0);
                ofRect(100, 100, 100, 100);
                if(foot_flag){
